...
function [eta_exct0, beta_optimum, beta_exct0, X, err, err_std, grad, grad_std, cost, cost_std,  Y_ex, Y_opt, Y_adj_opt, x0_inds] = ...
    data_assimil_bath_2020(N,n_obs,rand_x0,x0_min,dmu,ntrial,tmax,xmax,iter_max,line_min,smooth_grad,filt, tau_n, amp_bathy, ic_per, bathy_per, ...
    bathy_np_type, amp_ic, k_ic, k_bathy, phi_bathy, sigma, eta_shift, conj_grad_type, iter_chunk, a, c, cfl, noisy_obs,  print_iter)
...
%         disp('Periodic initial condition')
        ...
        eta_exct0 = amp_ic.*exp(-(X-eta_shift).^2/(0.1*xmax)^2);
...
%     disp('Periodic Bathymetry')
    beta_exct0 = amp_bathy*cos(k_bathy*2*pi/(2*xmax)*X+phi_bathy*ones(N,1));
     ...
     if bathy_np_type == 1
        beta_exct0 = amp_bathy*exp(-(X-0.5*xmax).^2/(sigma*xmax)^2);
