function [t,y] = RK34_forced_FOA_bath2(dt, u0, tRange, trend, h, bath, u_all, eta_all, dG_deta, dG_du)
% Usage:
...
for k = 1 :  numSteps
    ...
    U0 = y(:,k);
    U1 =  alpha(1,1)*U0                 + dt * beta(1,1)*trend(t,U0,h, bath, u_all(:,k), eta_all(:,k), dG_deta(:,k), dG_du(:,k) );
    U2 =  alpha(2,2)*U1                 + dt * beta(2,2)*trend(t,U1,h, bath, u_all(:,k), eta_all(:,k), dG_deta(:,k), dG_du(:,k) );
    U3 =  alpha(3,1)*U0 + alpha(3,3)*U2 + dt * beta(3,3)*trend(t,U2,h, bath, u_all(:,k), eta_all(:,k), dG_deta(:,k), dG_du(:,k) );
    U4 =  alpha(4,4)*U3                 + dt * beta(4,4)*trend(t,U3,h, bath, u_all(:,k), eta_all(:,k), dG_deta(:,k), dG_du(:,k) );
    y(:,k+1) = U4;
end
