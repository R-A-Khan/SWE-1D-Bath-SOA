...
for case_num = 1:3
    ...
    N_obs_all = [5 10 20 45];
    for l = 1:length(N_obs_all)
 
% Fixed parameters for the data assimilation calculations
cfl         = 1/3;
n_obs       = N_obs_all(l);% Number of observation points
...
x0_max      = 2.52;
dmu         = 0.12;     % Spacing between observation points
x0_min      = x0_max - (n_obs-1)*dmu ;   % Location of first observation point
...
str =sprintf('Results/SOAB1D11/DA Results/SOAB1D11_case_%d_N_obs_%d.mat', case_num, N_obs_all(l));
...
disp(sprintf('Case %d iteration %d with N_obs = %d completed', case_num, l, n_obs));
    end
end